// src/services/companyGrandLivreService.ts
...
// Hook pour une utilisation dans les composants React
export const useCompanyGrandLivreData = (companyId: string) => {
  ...
  useEffect(() => {
    try {
      const data = getCompanyGrandLivreData(companyId);
      setGrandLivreData(data);
    } catch (error) {
      console.error('Erreur de récupération des données de Grand Livre', error);
    }
  }, [companyId]);

  return grandLivreData;
};
